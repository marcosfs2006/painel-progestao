...
# Obtém a listagem dos RPPS existentes no Brasil - conforme API
#rpps <- cadastro %>% 
#  filter(regime == "RPPS") %>% 
#  select(-regime) %>% 
#  left_join(select(progestao, -ente, -uf), by="cnpj")

# Obtem a listagem dos RPPS existentes no Brasil - conforme ISP 2022
rpps_isp <- isp %>% 
...
```{r}
progestao %>% 
  filter(!is.na(nivel_inic), !is.na(dt_recertificacao)) %>% 
  count(nivel_inic, nivel_final) %>%
  adorn_totals("row") %>% 
  gt() %>% 
  cols_label(nivel_inic = "Nível Inicial",
             nivel_final = "Nível Final",
             n = "Qtd. de RPPS")
  #kbl(caption = "Mudanças de Nível nas Recertificações",
  #    col.names = c("Nível Inical", "Nível Atual", "Qtd. RPPS")) %>% 
...
progestao %>% 
  filter(nivel_final %in% c("I", "II", "III", "IV")) %>% 
  count(nivel_final, grupo) %>%
  rename("Nível Atual" = nivel_final) %>% 
...
### **Nivel e Perfil de Risco Atuarial**

```{r}

progestao %>% 
  filter(nivel_final %in% c("I", "II", "III", "IV")) %>% 
  count(nivel_final, perfil_atuarial) %>%
  rename("Nível Atual" = nivel_final) %>% 
